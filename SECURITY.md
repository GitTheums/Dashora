...
- **`esbuild` ≤ 0.24.2 (moderate, [GHSA-67mh-4wv8-2f99](https://github.com/advisories/GHSA-67mh-4wv8-2f99))** — pulled in transitively via `drizzle-kit@0.31.10` → `@esbuild-kit/esm-loader` (a deprecated package drizzle-kit still depends on as of its latest stable release). This is a **devDependency-only** chain: `drizzle-kit` is our migration-generation CLI, never bundled or run in production, and the advisory describes esbuild's own local dev server accepting arbitrary cross-origin requests — a mode Dashora never invokes. `drizzle-kit`'s only newer releases that drop this dependency are pre-1.0 release candidates; given the migration-tooling churn that would introduce, this has been assessed as low-risk and left unfixed for now rather than adopting an unstable release candidate of the schema-migration tool. Re-evaluate when `drizzle-kit` ships a stable release without the `@esbuild-kit/*` dependency.
