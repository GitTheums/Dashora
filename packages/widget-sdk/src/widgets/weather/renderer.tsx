...
import { WeatherBody, WeatherSkeleton } from "./body.js";
import type { WeatherConfig, WeatherData } from "./config.js";
import { WEATHER_WIDGET_ID } from "./definition.js";

export type WeatherRendererProps = WidgetRendererProps<WeatherData, WeatherConfig>;
...
export function WeatherRenderer({
  ...
  switch (state) {
    case "loading":
      body = <WeatherSkeleton layout={config.layout} />;
      break;
    case "refreshing":
      body = (
        <div style={widgetShellStyle}>
          <output style={widgetBannerStyle}>{message ?? "Refreshing…"}</output>
          {data ? <WeatherBody data={data} /> : <WeatherSkeleton layout={config.layout} />}
        </div>
      );
      break;
    case "success":
      body = data ? (
        ...
        <p style={widgetMutedStyle}>No forecast payload.</p>
      );
      break;
    case "empty":
      body = (
        <div style={widgetShellStyle}>
          ...
          <p style={widgetMutedStyle}>{message ?? "No weather data is available."}</p>
        </div>
      );
      break;
    case "stale":
      body = (
        <div style={widgetShellStyle}>
          ...
          {data ? <WeatherBody data={data} /> : null}
        </div>
      );
      break;
    case "error":
      ...
      body = (
        <div style={widgetShellStyle}>
          ...
          <p style={widgetMutedStyle}>
            {message ?? "Search for a location in settings before this widget can run."}
          </p>
        </div>
      );
      break;
  ...
  return (
    <WidgetFrame title={title} widgetId={WEATHER_WIDGET_ID} state={state} onRefresh={onRefresh}>
      {body}
    </WidgetFrame>
  );
}
