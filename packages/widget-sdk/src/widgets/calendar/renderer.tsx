...
import { CalendarBody, CalendarSkeleton } from "./body.js";
import type { CalendarConfig, CalendarData } from "./config.js";
import { CALENDAR_WIDGET_ID } from "./definition.js";

export type CalendarRendererProps = WidgetRendererProps<CalendarData, CalendarConfig>;
...
export function CalendarRenderer({
  ...
  switch (state) {
    case "loading":
      body = <CalendarSkeleton layout={config.layout} />;
      break;
    case "refreshing":
      body = (
        <div style={widgetShellStyle}>
          <output style={widgetBannerStyle}>{message ?? "Refreshing…"}</output>
          {data ? <CalendarBody data={data} /> : <CalendarSkeleton layout={config.layout} />}
        </div>
      );
      break;
    case "success":
      body = data ? (
        ...
        <p style={widgetMutedStyle}>No calendar payload.</p>
      );
      break;
    case "empty":
      body = (
        <div style={widgetShellStyle}>
          <p style={{ margin: 0, fontSize: "1rem", fontWeight: 600 }}>Nothing upcoming</p>
          <p style={widgetMutedStyle}>
            {message ?? "No events in the configured look-ahead period."}
          </p>
        </div>
      );
      break;
    case "stale":
      body = (
        <div style={widgetShellStyle}>
          ...
          {data ? <CalendarBody data={data} /> : null}
        </div>
      );
      break;
    case "error":
    ...
    case "configuration-required":
      body = (
        <div style={widgetShellStyle}>
          ...
          <p style={widgetMutedStyle}>
            {message ?? "Add at least one ICS feed URL in settings before this widget can run."}
          </p>
        </div>
      );
      break;
  ...
  return (
    <WidgetFrame title={title} widgetId={CALENDAR_WIDGET_ID} state={state} onRefresh={onRefresh}>
      {body}
    </WidgetFrame>
  );
}
