...
import type { HackerNewsData, HackerNewsItem, HackerNewsLayout } from "./config.js";
...
export function HackerNewsSkeleton({ layout = "rich" }: { layout?: HackerNewsLayout }) {
  ...
  return (
    <div
      ...
      aria-busy="true"
      aria-live="polite"
      aria-label="Loading Hacker News"
    >
      {Array.from({ length: rows }, (_, index) => `hn-skel-${index}`).map((id, index) => (
...
function StoryMeta({ item, data }: { item: HackerNewsItem; data: HackerNewsData }) {
  ...
  if (data.showCommentCount) {
    parts.push(
      <a key="comments" {...linkProps(item.hnUrl, data.openInNewTab)} style={{ color: "inherit" }}>
        {item.commentCount} comment{item.commentCount === 1 ? "" : "s"}
      </a>,
    );
  }
  if (item.domain) {
    parts.push(<span key="domain">{item.domain}</span>);
  }
...
function StoryRow({
  ...
  const primaryHref = item.url ?? item.hnUrl;
  const title = (
    <a
...
export function HackerNewsBody({ data }: { data: HackerNewsData }) {
  return (
    <VirtualList
      items={data.items}
