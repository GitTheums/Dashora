...
import { HackerNewsBody, HackerNewsSkeleton } from "./body.js";
import type { HackerNewsConfig, HackerNewsData } from "./config.js";
import { HACKER_NEWS_WIDGET_ID } from "./definition.js";

export type HackerNewsRendererProps = WidgetRendererProps<HackerNewsData, HackerNewsConfig>;

export function HackerNewsRenderer({
  title,
  config,
  state,
  data,
  message,
  onRefresh,
}: HackerNewsRendererProps) {
  let body: ReactNode;
  switch (state) {
    case "loading":
      body = <HackerNewsSkeleton layout={config.layout} />;
      break;
    case "refreshing":
      body = (
        <div style={widgetShellStyle}>
          <output style={widgetBannerStyle}>{message ?? "Refreshing…"}</output>
          {data ? <HackerNewsBody data={data} /> : <HackerNewsSkeleton layout={config.layout} />}
        </div>
      );
      break;
    case "success":
      body = data ? (
        <HackerNewsBody data={data} />
      ) : (
        <p style={widgetMutedStyle}>No Hacker News payload.</p>
      );
      break;
    case "empty":
      body = (
    ...
    case "stale":
      body = (
        <div style={widgetShellStyle}>
          ...
          {data ? <HackerNewsBody data={data} /> : null}
        </div>
      );
      break;
    case "error":
  ...
  return (
    <WidgetFrame title={title} widgetId={HACKER_NEWS_WIDGET_ID} state={state} onRefresh={onRefresh}>
      {body}
    </WidgetFrame>
  );
}
